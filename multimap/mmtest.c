...
int main() {
    ...
    /*
    printf("\nChecking key removal.\n");
    for (i = 0; remove_values[i] != -1; i += 3) {
        int answer = remove_values[i + 2];
        int remove = mm_remove_pair(mm, remove_values[i], remove_values[i + 1]);

        printf(" * (%d, %d) should%s have been removed:  %s",
            remove_values[i], remove_values[i + 1], answer ? "" : " NOT",
            answer ? "    " : "");

        if ((remove && answer) || (!remove && !answer)) {
            printf("PASS");
        }
        else {
            printf("FAIL");
    ...
    printf("\nProbing multimap for pairs after removal.\n");
    for (i = 0; remove_probe_values[i] != -1; i += 3) {
        int answer = remove_probe_values[i + 2];
        int probe = mm_contains_pair(mm,
            remove_probe_values[i], remove_probe_values[i + 1]);
